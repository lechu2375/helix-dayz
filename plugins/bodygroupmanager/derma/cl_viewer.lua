...
function PANEL:Display(target)
	...
	self.saveButton.DoClick = function()
		...
		net.Start("ixBodygroupTableSet")
			...
			net.WriteUInt(self.model.Entity:GetSkin(), 4)
		net.SendToServer()
	end
...
function PANEL:AddSkinButton()
    -- Disregard the model bodygroup.
        ...
        self.skinLabel = self.skinBox:Add("DLabel")

        self.skinLabel:SetText("Skin")
        self.skinLabel:SetFont("ixMediumFont")
        self.skinLabel:Dock(LEFT)
        self.skinLabel:DockMargin(30, 0, 0, 0)
        self.skinLabel:SetWidth(200)

        self.skinNextButton = self.skinBox:Add("DButton")

        self.skinNextButton:Dock(RIGHT)
        self.skinNextButton:SetText("Next")
        self.skinNextButton.DoClick = function()
            local current = self.model.Entity:GetSkin()
            if (current+1>self.model.Entity:SkinCount()) then
                return
            end
            current=current+1
            self.model.Entity:SetSkin(current)
			self.skinPreviousLabel:SetText(self.model.Entity:GetSkin())
        end

        self.skinPreviousLabel = self.skinBox:Add("DLabel")


        self.skinPreviousLabel:SetText(self.model.Entity:GetSkin())
        self.skinPreviousLabel:SetFont("ixMediumFont")
        self.skinPreviousLabel:Dock(RIGHT)
        self.skinPreviousLabel:SetContentAlignment(5)

        self.skinPreviousButton = self.skinBox:Add("DButton")

        self.skinPreviousButton:Dock(RIGHT)
        self.skinPreviousButton:SetText("Previous")
        self.skinPreviousButton.DoClick = function()
            local current = self.model.Entity:GetSkin()
            if (current-1<0) then
                return
            end
