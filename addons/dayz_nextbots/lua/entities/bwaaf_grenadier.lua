...
if SERVER then
	...
	function ENT:PrimaryFire()
		if not self:HasWeapon() or self.readytofire or self:GroundDistance(self:GetEnemy()) < 512 then return end
		self:CallInCoroutineOverride(function(self)
			self.readytofire = true
			self:PlaySequenceAndMove('cidle_rpg', 0.5)
			self.readytofire = false
			self:WeaponPrimaryFire(self:GetShootAnimation())
		end)
	end

	function ENT:Reload()
		if not self:HasWeapon() then return end
		return self:WeaponReload(ACT_HL2MP_GESTURE_RELOAD_REVOLVER)
	end
