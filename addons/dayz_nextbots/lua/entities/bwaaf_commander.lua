...
-- Misc --
ENT.PrintName = "BWAAF Commander"
ENT.Category = "BlackWatch Army"
ENT.Models = {
  	"models/blackwatcharmy_nextbots/bwaaf_commander_ragdoll.mdl"
}
...
if SERVER then
    ...
    function ENT:CustomInitialize()
		self:GiveWeapon("bwa_m4a1_acog")
		self.AvoidEnemyRange = 1200
		self.AvoidAfraidOfRange = 1200
      	self:SetDefaultRelationship(D_HT)
		if GetConVar("bwa_friends"):GetBool() then
			self:AddRelationship("player D_LI 99")
		end
      	self:SetHealth(math.random(200,250))
      	self:WeaponReload()
		...
		self:SetBodygroup(9, math.random(0,4))
		self:SetBodygroup(10, math.random(0,5))
		...
		self:SetBodygroup(17, math.random(0,1))
		self.currentsound = ""
		self.bwa = true
		self.GrenadeCount = math.random(4,8)
		self.Covering = false
		self.Cover_Side_left = false
		self.Cover_Pos = nil
		self.Cover_Pos_Valid = false
		self.Covering_Anim = false
		self:SetCooldown("grenades", math.random(5,15))
		self:SetCooldown("support", math.random(15,30))
		self:SetCooldown("support2", math.random(15,30))
	...
	function ENT:CallSupport(type)
		local pos = self:RandomPos(256,512)
		...
		if tr.HitSky then
			if type == "heli" then
				...
				local sup = ents.Create('bwa_infil')
				sup:SetPos(pos)
				sup:Spawn()

				self:CallInCoroutineOverride(function(self)
					self:VoiceLine("inbound")
					self:PlaySequenceAndMove('seq_wave_smg1')
				end)
			else
				self:SetCooldown("support2", math.random(30,45))
				BWA:CallAirstrike(self:GetEnemy())
				self:CallInCoroutineOverride(function(self)
					self:VoiceLine("inbound")
					self:PlaySequenceAndMove('seq_wave_smg1')
				end)
	...
	function ENT:OnTakeDamage(dmg)
		...
		if self:Health() > 0 and dmg:GetDamage() >= self:Health() and not self.laststand then
			self.laststand = true
			dmg:SetDamage(0)
			self:LastStand()
			return 0
	...
	function ENT:VoiceLine(type)
		...
		elseif type == "reload" then
			sounds = 'bwaaf/reload'..math.random(1,6)..'.wav'
		elseif type == "grenade" then
			sounds = 'bwaaf/grenade'..math.random(1,2)..'.wav'
		elseif type == "inbound" then
			sounds = 'bwaaf/inbound'..math.random(1,4)..'.wav'
		end
	...
	function ENT:CheckGuard()
		for _, v in ipairs(self.guards) do
			if not IsValid(v) then
				table.RemoveByValue(self.guards, v)
			end
		end
		if #self.guards > 2 then return end

		local tab = ents.FindByClass('bwaaf_soldier')
		for _, ent in ipairs(tab) do
			if not ent.FollowTarget and not table.HasValue(self.guards, ent) and #self.guards < 3 then
				ent:Follow(self)
			end
		end
	end
	...
	function ENT:CustomThink()
		...
		if self:GetCooldown("support") == 0 and IsValid(self:GetEnemy()) and BWA:CountAlives() < 8 then
			self:CallSupport("heli")
		end
		if self:GetCooldown("support2") == 0 and IsValid(self:GetEnemy()) and self:GetEnemy():GetPos():DistToSqr(self:GetPos()) > 1024^2 then
			self:CallSupport("air")
		end
		if self:GetCooldown("guards") == 0 and GetConVar("bwa_enable_follow_to_commander"):GetBool() then
			self:CheckGuard()
		end
