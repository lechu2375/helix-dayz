if not DrGBase then return end -- return if DrGBase isn't installed
SWEP.Base = "drgbase_weapon" -- DO NOT TOUCH (obviously)

-- Misc --
SWEP.PrintName = "RPG"
SWEP.Class = "bwa_rpg"
SWEP.Spawnable = false

-- Looks --
SWEP.HoldType = "rpg"
SWEP.ViewModelFOV	= 54
SWEP.ViewModelFlip = false
SWEP.ViewModelOffset = Vector(0, 0, 0)
SWEP.ViewModelAngle = Angle(0, 0, 0)
SWEP.UseHands = false
SWEP.WorldModel = "models/bwa_wep/atrg.mdl"
SWEP.WMPos = Vector( -1, 6, 4)
SWEP.WMAng = Vector(-5, 0, 180)

-- Primary --

-- Shooting
SWEP.Primary.Damage = 0
SWEP.Primary.Bullets = 0
SWEP.Primary.Spread = 0
SWEP.Primary.Automatic = false
SWEP.Primary.Delay = 1
SWEP.Primary.Recoil = 0

-- Ammo
SWEP.Primary.Ammo	= "RPG_Round"
SWEP.Primary.Cost = 1
SWEP.Primary.ClipSize	= 1
SWEP.Primary.DefaultClip = 1

-- Effects
SWEP.Primary.Sound = "weapons/rpg/rocketfire1.wav"
SWEP.Primary.EmptySound = "Weapon_AR2.Empty"

if CLIENT then
    function SWEP:DrawWorldModel()     
        wm = ClientsideModel(self.WorldModel)

        if IsValid(wm) then
            if IsValid(self.Owner) then
                pos, ang = self.Owner:GetBonePosition(self.Owner:LookupBone("ValveBiped.Bip01_R_Hand"))
                    
                if pos and ang then
                    ang:RotateAroundAxis(ang:Right(), self.WMAng[1])
                    ang:RotateAroundAxis(ang:Up(), self.WMAng[2])
                    ang:RotateAroundAxis(ang:Forward(), self.WMAng[3])
    
                    pos = pos + self.WMPos[1] * ang:Right()
                    pos = pos + self.WMPos[2] * ang:Forward()
                    pos = pos + self.WMPos[3] * ang:Up()
    
                    wm:SetRenderOrigin(pos)
                    wm:SetRenderAngles(ang)
                    wm:DrawModel()
                end
            else
                wm:SetRenderOrigin(self:GetPos())
                wm:SetRenderAngles(self:GetAngles())
                wm:DrawModel()
                wm:DrawShadow()
            end
    
            wm:SetupBones()
            wm:Remove()
        else
            self:DrawModel()
        end
    end
else    
    function SWEP:PrimaryAttack()
        if self:Clip1() <= 0 then return end

        local proj = ents.Create("proj_bwa_rocket")
        local ply_Ang = self:GetOwner():GetAimVector():Angle()
        local ply_Pos = self:GetOwner():GetShootPos() + ply_Ang:Forward()*32+ ply_Ang:Up()*8 + ply_Ang:Right()*10
        proj:SetPos(ply_Pos)
        proj:SetAngles(ply_Ang)
        proj:SetOwner(self:GetOwner())
        proj:Spawn()

        self.Weapon:SetClip1( self.Weapon:Clip1() - 1 )	
    end
end

-- DO NOT TOUCH --
AddCSLuaFile()
DrGBase.AddWeapon(SWEP)