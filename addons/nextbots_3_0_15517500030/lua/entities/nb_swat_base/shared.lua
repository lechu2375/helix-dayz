	...
	while ( path:IsValid() ) do
		...
		if !self.Downed then
			if ( self.loco:IsStuck() ) then
				...
				if(self.JumpDelay<CurTime()) then
					...
					local jumpstate = self:GetJumpBlockState(dir)

					if jumpstate then
						local pos = self:GetPos()
						self.loco:Approach(pos-dir*200,1)
					end
						...
						if !self.Reloading then
							...
							if ( pos ) and enemy:IsValid() and enemy:Health() > 0 then
								...
								if ( self:GetRangeSquaredTo( enemy ) < self.StopRange*self.StopRange and self:GetRangeSquaredTo( enemy ) > self.BackupRange*self.BackupRange and self:Visible( self.Enemy ) and !self.Crouching ) then
									...
									if((self.NextFail or CurTime())<=CurTime()) then 
										local result = self:ChaseEnemy( opts )
										if(result=="failed" or result=="nil") then
