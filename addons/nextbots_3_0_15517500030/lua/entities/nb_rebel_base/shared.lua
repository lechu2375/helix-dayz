...
function ENT:ChaseEnemy( options )
	...
	if(self.ChaseDistance) then
		if(self.ChaseDistance*self.ChaseDistance<self:GetRangeSquaredTo(enemy)) then
			return "failed"
		end
	end
	...
	while ( path:IsValid() and self:HaveEnemy() and ( enemy and enemy:IsValid() and enemy:Health() > 0) ) do
		...
		if ( !self.Reloading and !self.loco:IsStuck() and ( enemy and enemy:IsValid() and enemy:Health() > 0) ) then
				...
				if ( path:GetAge() > options.maxage ) then	
					print("compute?")
					local enemyNav = IsValid(navmesh.GetNearestNavArea( enemy:GetPos(),false,100, false)) 
					if(!enemyNav)then
						print("No navmesh for enemy!")
					end
					if( (self.NextFail or CurTime())<=CurTime() or !self.DontChase or enemyNav) then 
						local result = path:Compute( self, enemy:GetPos(), function( area, fromArea, ladder, elevator, length )
					...
					if(!result or result=="false" or result=="nil") then
						//print("Delay for compute")
						self.NextFail = CurTime()+10
						if(self.LastFail==enemy) then
							print("Again, cant calc path to him.")
							//self.Enemy = false
							self.DontChase = true
						end
						self.LastFail = enemy
...
function ENT:Jump()
	...
	self:StartMovementAnim( self.JumpAnim, self.Speed )
...
function ENT:GetJumpBlockState(dir)
	...
	if tr.Hit then
		local maxjump = self.JumpHeight-step
...
function ENT:TryStuck(pos,t,tr)
	t.start = pos
	t.endpos = pos
	...
	for z=0,w*1.2,w*0.2 do
		for x=0,w*1.2,w*0.2 do
			for y=0,w*1.2,w*0.2 do
				if self:TryStuck(pos+Vector(x,y,z),t,tr) then return end
				if self:TryStuck(pos+Vector(-x,y,z),t,tr) then return end
				if self:TryStuck(pos+Vector(x,-y,z),t,tr) then return end
				if self:TryStuck(pos+Vector(-x,-y,z),t,tr) then return end
				if self:TryStuck(pos+Vector(x,y,-z),t,tr) then return end
